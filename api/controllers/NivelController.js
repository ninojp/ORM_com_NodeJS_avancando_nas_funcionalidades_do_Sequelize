...
class NivelController{
  ...
  static async criaNivel(req, res){
    ...
    try{
      const novoNivelCriada = await niveisServices.criaRegistro(novoNivel);
  ...
  static async apagaNivel(req, res){
    ...
    try{
      ...
      return res.status(200).json({mensagem: `O Nivel de ID: ${id}, foi deletado`});
    }catch(error){
      return res.status(500).json(error.message);
    }
  }
  ...
  static async restauraNivel(req, res){
    ...
    try{
      ...
      return res.status(200).json({mensagem: `O Nivel de ID: ${id}, foi Restaurada!`});
    }catch(error){
      return res.status(500).json(error.message);
    }
  }
